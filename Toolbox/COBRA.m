...
fva::GurobiSolver = "The Gurobi solver has been removed from the MASS Toolbox. Please use another solver.";
Options[fva]=updateRules[FilterRules[Options[fba],Except[{"OptFlag"}]],{"ProgressBar"->True}];
fva[model_MASSmodel,bounds:({_Rule..}|{}):{},opts:OptionsPattern[]]:=Module[{maxTmp,minTmp,result,progressBarQ,rxns,i,j},
	If[OptionValue["Solver"]===GurobiFVA,
		(*result=GurobiFVA[model,bounds]*)Message[fva::GurobiSolver];Abort[],
		progressBarQ=If[$FrontEnd=!=Null,OptionValue["ProgressBar"],False];
		rxns=getID/@model["Fluxes"];
...
GAMS::wrongSolverSpec="Something is wrong with the provided solver specification. Please, check your optimization problem category (Category -> NLP, LP, MILP, etc.) and solver specifiation (e.g. Solver-> CONOPT etc.).";
